<%@ Page Language="C#" AutoEventWireup="true" CodeFile="PictureManage.aspx.cs" Inherits="dptopfo_PictureManage" %>
...
function modifyNavigate(pid,name)
{
    ...
    url="ImageUpdate.aspx?ImageID="+pid+"&LoginName="+name;
                ...
                <asp:Repeater ID="NewsList" OnItemDataBound="NewsList_ItemDataBound" runat="server">
                    <ItemTemplate>
                        <tr onmouseover="this.style.backgroundColor='#f5f5f5';" onmouseout="this.style.backgroundColor=''">
                           <%-- <td >
                                <label>
                                    <input type="checkbox" name="cbxSelect" id="cbxSelect" value="<%#Eval("imageid")%>" />
                                </label>
                            </td>--%>
                            <td>
                                <%#Eval("imageid")%>
                            ...
                            <td>
                                <%#Convert.ToDateTime(Eval("createdatetime")).ToString("yyyy-MM-dd (hh:mm)")%>
                            </td>
                            <td>
                            <a href='http://<%#Eval("loginName") %>.topfo.com/Photogallery.htm'; target="_blank">查看</a> 
                                <a href='JavaScript:modifyNavigate("<%#Eval("imageid") %>","<%#Eval("loginName") %>");'>修改</a> 
                                <%--<a href='JavaScript:DelNav("<%#Eval("imageid") %>");');' onclick="return confirm('确认要删除吗？') ">删除</a>--%>
