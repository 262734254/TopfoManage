...
<%@ Register Src="CheckComPublishInfo.ascx" TagName="CheckComPublishInfo" TagPrefix="uc1" %>
<%@ Register Src="CheckConflict.ascx" TagName="CheckConflict" TagPrefix="uc2" %>
<style type="text/css">
.tabbiank menu{
	width: 80px;
	float: left;
...
.tabbiank .menulw{
	float: left;
	width: 45px!important;width:60px;
}
.tabbiank strong{
	color: #333333;
}
...
<table width="100%" border="0" align="center" bgcolor="#4DA6FF" cellpadding="1" cellspacing="1"
    class="tabbiank">
    <tr>
    ...
    <tr>
        ...
        <td width="625" align="left" bgcolor="#EFF6FF">
            <asp:TextBox ID="txtCompanyName" onchange="CAcheckNavigate(1);" runat="server" Width="324px" />
            <input id="Button1" class="buttomal" onclick="openList()" style="width: 172px" type="button"
                value="查询该公司发布的融资信息" /><span id="spCAComName"></span>
            <div class="clear">
            </div>
        </td>
    </tr>
    <tr id="Tr1" name="trswitchpublish">
        <td width="133" align="right" bgcolor="#F7F7F7" style="height: 26px">
            <strong>项目承办单位：</strong></td>
        <td width="625" bgcolor="#EFF6FF" align="left" style="height: 26px">
        ...
        <td width="625" align="left" bgcolor="#EFF6FF">
            <div id="divContactMan" runat="server">
                <div id="divInit" runat="server">
                    姓名：<asp:TextBox ID='txtName' onchange="CAcheckNavigate(2)" Width="150px" runat="server" />&nbsp;&nbsp;
                    手机：<asp:TextBox ID='txtMobile' Width="150px" runat="server" />
                    <input type="button" onclick="JavaScript:<%=this.ClientID %>_AddContact.add();" class="buttomal"
                        value="添加更多联系人" style="width: 114px" />
                    <%=this.LinkOutputHTML %>
                </div>
            </div>
            ...
            请填写该项目的直接负责人真实姓名，姓名必填，手机号码与电话必填一项
    ...
    <tr>
        ...
        <td width="625" align="left" bgcolor="#EFF6FF">
            ...
            <asp:TextBox ID="txtTelNumber" runat="server" size='18' /><br />
            <span id="spTel"></span>
            <span id="spContact"></span>
            <div class="clear">
            </div>
    ...
    <tr id="Tr2" name="trswitchpublish">
        ...
        <td width="625" align="left" bgcolor="#EFF6FF">
            <asp:TextBox ID="txtEmail" onchange="CAcheckNavigate(4)" runat="server" size='18'
                Width="269px" /><br />
...
<script type="text/javascript" language="javascript">
      function openList()
    {
        var infotype="merchant";
        var id="<%=this.txtCompanyName.ClientID %>";
        var name=document.getElementById(id).value;
        var url="infolistbycompany.aspx?comname="+name+"&infotype="+infotype;
        window.open(url,"newwindow","height=500,width=710,top=150,left=200,toolbar=no,menubar=no,scrollbars=no,resizable=no,location=no,status=no");
    }
...
//			obj.insertAdjacentHTML("beforeEnd",str);
	...
	AddContact.prototype.checkComName = function(){
	    ...
	    //if(!this.checkByteLength(value,1,60)){
	    if(!cTrim(value,0)){
	        document.getElementById("spCAComName").innerHTML = "&nbsp;&nbsp;&nbsp;请正确填写招商机构名称！";
	...
	AddContact.prototype.checkLinkMan = function(){
    	...
    	{
    	    document.getElementById("splinkMan").innerHTML = "&nbsp;&nbsp;&nbsp;请正确的填写联系人姓名！";
...
//        var spMsgID = this.Prefix + "SP" + i.toString();
    ...
    {      
        var flag=true;
        var filterNum =  /[0-9]{3,5}/;///^[0-9]+$/;
        var filterNum1 = /^[0-9,\-]+$/;
        var codeObj=document.getElementById("<%=this.txtTelZoneCode.ClientID %>");
        var numObj=document.getElementById("<%=this.txtTelNumber.ClientID %>");
        if(codeObj.value!=""||numObj.value!="")
        {
            if(!filterNum.test(codeObj.value)||!filterNum1.test(numObj.value))
            {
                document.getElementById("spTel").innerHTML = "&nbsp;&nbsp;&nbsp;区号或电话号码错误...";
                document.getElementById("spTel").className = "noteawoke";
                flag=false;
            }
            else
    ...
    {      
        var flag=true;
        var filterNum = /^[0-9]+$/;
        var mobileObj=document.getElementById("<%=this.txtMobile.ClientID %>");
        
        if(mobileObj.value!="")
        {
            if(!filterNum.test(mobileObj.value)||mobileObj.value.length!=11)
            {
                document.getElementById("spMobile").innerHTML = "&nbsp;&nbsp;&nbsp;手机号码错误...";
    ...
    function checkContact()
    {
        ...
        if(codeObj.value==""&&numObj.value=="")
            ...
            codeStr=codeObj.value+"-"+numObj.value;
        if(codeStr==""&&mobileObj.value=="")
        {
            document.getElementById("spContact").innerHTML = "&nbsp;&nbsp;&nbsp;固话号码与手机号码必填一项...";
            document.getElementById("spContact").className = "noteawoke";
            codeObj.focus();
            flag=false;
        }
        else
        {
            document.getElementById("spContact").innerHTML = "";
            document.getElementById("spContact").className = "";
            if(telFlag&&mobileFlag)
            {
                flag=true;
            }
            else
