...
namespace Tz888.Common
{
    ...
    public class Common
    {
        ...
        /// <returns></returns>
        public bool bBuyInfoOfUserName( string InfoID)
        {
            ...
            DataSet ds = Tz888.DBUtility.DbHelperSQL.RunProcedure("CheckBuyByUser", parameters, "ds");
        ...
        #region 根据输入的文件名获取信息ID(InfoID)
        public static long GetInfoIDForString(string FilePath)
        {
            if (FilePath == "")
                return 0;
            string strTmp = FilePath;
            long InfoID = 0;

            string strTmp1 = "";
            for (int i = 0; i < strTmp.Length; i++)
            {
                string strTimp2 = strTmp[i].ToString().Trim().ToLower();
                if (strTimp2 != "")
                {
                    strTmp1 = strTmp1 + strTimp2;
                }
            }

            string strCopmare = "?infoid=";
            int Index = strTmp1.IndexOf("?infoid=");
            if (Index >= 0)
            {
                strTmp1 = strTmp1.Substring(Index + strCopmare.Length);
                InfoID = Convert.ToInt64(strTmp1);
            }
            else
            {
                //获取文件名称
                strTmp = strTmp.Substring(strTmp.LastIndexOf("/"), (strTmp.Length - strTmp.LastIndexOf("/")));
                strTmp = strTmp.Substring(strTmp.IndexOf("_") + 1, strTmp.IndexOf(".") - strTmp.IndexOf("_") - 1);
                if (strTmp.IndexOf("_") > 0)
                {
                    strTmp = strTmp.Substring(0, strTmp.IndexOf("_"));
                    InfoID = Convert.ToInt64(strTmp);
                }
                else
                {
                    InfoID = Convert.ToInt64(strTmp);
