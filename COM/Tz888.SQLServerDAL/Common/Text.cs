...
{ /// <summary>
    ...
    public class Text
    {
        ...
        public string ThrowHtml(string sStr)
        {
            ...
            sStr = sStr.Replace("&lt;p &gt;", "").Replace("&lt;/p &gt;", "");
            sStr = sStr.Replace("<br>", "").Replace("<br/>", "");
        ...
        /// <summary>
        /// 如果数据库中MemberGradeID=1002(拓富通会员)，则返回logo地址
        /// </summary>
        /// <param name="str"></param>
        /// <returns></returns>
        public static string GetLogoLink(string str)
        {
            if (str.Trim() == "1002")
                return "<img src=\"http://www.topfo.com/Web13/images/home/tfzs.gif\" border=\"0\"  alt=\"拓富通会员\" />";
            else
                return "";
        }

        public static string GetLabelLink(string str, string strLinkFile)
        {
            //标签：<a href="AllResource.aspx?Keyword=<%# Eval("keyword") %>"  target="_blank" ><%# Eval("keyword")%> </a>

            string strRetu = "";
            if (str.Trim() != "")
            {
                strRetu += "标签：<a href=" + strLinkFile.Trim() + "?Keyword=" + str.Trim() + " target=_blank >" + str.Trim() + "</a>";
            }
            return strRetu;
        }

        /// <summary>
        /// 如果title为空，则显示ShortTitle
        /// </summary>
        /// <param name="strTitle"></param>
        /// <param name="strShortTitle"></param>
        /// <returns></returns>
        public static string GetTitle(string strTitle, string strShortTitle)
        {
            if (strTitle.Trim() != "")
                return strTitle;
            else
        ...
        /// <summary>
        /// 如果内容1不为空，则显示内容1，否则显示内容2
        /// </summary>
        /// <param name="strContact1"></param>
        /// <param name="strContact2"></param>
        /// <returns></returns>
        public static string GetContact(string strContact1, string strContact2)
        {
            if (strContact1.Trim() != "")
                return strContact1.Replace("<br>", "");
            else
                return strContact2.Replace("<br>", "");
        }
        ...
        /// <returns></returns>
        public static string FormatNull(string strFile, int iLong)
        {
        ...
        /// 行业
        /// </summary>
        /// <param name="strInvest"></param>
        /// <returns></returns>
        public static string GetIndustryBName(string strInvest)
        {
            string[] strIndustry = strInvest.Split(',');
            string str = "";
            if (strIndustry.Length > 0)
            {
                for (int i = 0; i < strIndustry.Length; i++)
                {
                    str += strIndustry[i].ToString() + "<br />";
                }
            }
            else
            {
        ...
        /// <returns></returns>
        public static string GetArea(string ProvinceName, string CityName, string CountyName)
        {
            ProvinceName = ProvinceName.Replace(",", "");
            CityName = CityName.Replace(",", "");
            CountyName = CountyName.Replace(",", "");
            return ProvinceName + " " + CityName + "<br />" + CountyName;
        }
        ...
        /// <param name="str"></param>
        /// <returns></returns>
        public static string GetConfirmLink(string str)
        {
            if (str == "1")  //确认资源
                return "<a href=\"http://www.topfo.com/help/otherone.shtml\"  target=\"_blank\"><img src=\"img/res3_rz.gif\" width=\"65\" height=\"55\"  alt=\"已认证资源\" /></a>";
            else //末确认资源
                return "<a href=\"http://www.topfo.com/help/otherone.shtml\"  target=\"_blank\"><img src=\"img/res3_icon4_gray.gif\" width=\"65\" height=\"55\" alt=\"未认证资源\" /></a>";
        }
        ...
        /// </summary>
        /// <param name="str"></param>
        /// <param name="iType">1为yyy-mm-dd</param>
        /// <returns></returns>
        public static string FormatDateType(string str, int iType)
        {
            string strRetu = "";
            try
            {
                ...
                if (iType == 1)
                {
                    strRetu = dt.ToString("yyyy-MM-dd");
                }
            }
            catch
            {
        ...
        /// <param name="str"></param>
        /// <returns></returns>
        public static string FormatDataToBfh(string str)
        {
            if (str.Trim() != "")
                return str + "%";
            else
        ...
        /// <summary>
        /// 搜索title及comabout两字段，组合成sql语句[重构一个，是最新搜索结查],折字程序
        /// </summary>
        /// <param name="strSearch"></param>
        /// <param name="strKeyWord"></param>
        /// <returns></returns>
        public static string FormatSearchStr_Ictclass(string strSearch)
        {
            if (strSearch == "")
                return "";
            else
            {
                string strTitle = "";
                string strComAbout = strSearch.Replace(",", "%' or ComAbout like '%");
                strComAbout = " and( ComAbout like '%" + strComAbout + "%')  ";

                strTitle = strComAbout.Replace("ComAbout", "title");
                strSearch = strComAbout + strTitle;
            }
            return strSearch;
        }
        ...
        /// 格式化字符样式
        /// </summary>
        /// <param name="str">格式化的字符</param>
        /// <param name="strKeyWord">关键字</param>
        /// <returns></returns>
        public static string FormatSearchFontSytle(string str, string strKeyWord)
        {
            if (strKeyWord.Trim() == "")
                return str;
            strKeyWord = FormatDh(strKeyWord, ",");
            strKeyWord = FormatDh(strKeyWord, "，");
            string[] arr = strKeyWord.Split(',');
            for (int i = 0; i < arr.Length; i++)
            {
                str = str.Replace(arr[i].ToString(), "<font color=\"#ff6600\" ><b>" + arr[i].ToString() + "</b></font>");
            }
            return str;
        }
        ...
        #region[防注入法]
        public static string ReplInject(string str)
        {
            if (str == "")
                return "";
        ...
        //{
        //    string keyword = str;
        //    string[] mykeyword = keyword.Split(' ');
        //    System.Text.StringBuilder stbkeyWord = new System.Text.StringBuilder();
        //    if (mykeyword.Length > 0 && keyword != "")
        //    {
        //        stbkeyWord.Append("<span style=\"color:#000000\">标签：</span>");

        //        for (int i = 0; i < mykeyword.Length; i++)
        //        {
        //            if (ViewState["keyWord"].ToString().Trim() == "")
        //            {
        //                if (mykeyword[i].Length < 6)
        //                {
        //                    mykeyword[i] = "<a title=" + mykeyword[i] + " class=\"ablue01\" href='SearchResultProject.aspx?KeywordUrl=" + mykeyword[i] + "' target=\"_parent\"><span>" + mykeyword[i] + "</span></a>";
        //                }
        //                else
        //                {
        //                    string word = mykeyword[i].ToString();
        //                    mykeyword[i] = mykeyword[i].Substring(0, 4) + "..";
        //                    mykeyword[i] = "<a title=" + word + " class=\"ablue01\" href='SearchResultProject.aspx?KeywordUrl=" + word + "' target=\"_parent\"><span>" + mykeyword[i] + "</span></a>";
        ...
        //                if (mykeyword[i].Length < 6)
        //                {
        //                    if (mykeyword[i].IndexOf(ViewState["keyWord"].ToString(), 0) == -1)
        //                    {
        //                        mykeyword[i] = "<a title=" + mykeyword[i] + " class=\"ablue01\" href='SearchResultProject.aspx?KeywordUrl=" + mykeyword[i] + "' target=\"_parent\"><span>" + mykeyword[i] + "</span></a>";
        //                    }
        //                    else
        //                    {
        //                        mykeyword[i] = "<a title=" + mykeyword[i] + " class=\"aorange04\" href='SearchResultProject.aspx?KeywordUrl=" + mykeyword[i] + "' target=\"_parent\"><span>" + mykeyword[i] + "</span></a>";
        //                    }
        //                }
        //                else
        //                {
        //                    string word = mykeyword[i].ToString();
        //                    mykeyword[i] = mykeyword[i].Substring(0, 4) + "..";
        //                    if (word.IndexOf(ViewState["keyWord"].ToString(), 0) == -1)
        //                    {
        //                        mykeyword[i] = "<a title=" + word + " class=\"ablue01\" href='SearchResultProject.aspx?KeywordUrl=" + word + "' target=\"_parent\"><span>" + mykeyword[i] + "</span></a>";
        //                    }
        //                    else
        //                    {
        //                        mykeyword[i] = "<a title=" + word + " class=\"aorange04\" href='SearchResultProject.aspx?KeywordUrl=" + word + "' target=\"_parent\"><span>" + mykeyword[i] + "</span></a>";
        //                    }

        //                }
        //            }
        //            stbkeyWord.Append(mykeyword[i]);
        //            if (i < mykeyword.Length - 1)
        //            {
        //                stbkeyWord.Append(" | ");
        //            }
        //        }
        //    }
        //    return stbkeyWord.ToString();
        //}
        //#endregion
