...
namespace Tz888.SQLServerDAL
{
    class loansInfoTab : IloansInfoTab
    {
        ...
        {
            string sql = "select * from loansInfoTab where LoginName=@LoginName";
            List<Tz888.Model.LoansInfoTab> list = new List<LoansInfoTab>();
            DataSet set = null;
            set = DbHelperSQL.Query(sql, new SqlParameter("@LoginName", name));
        ...
        {
            string sql = "Update loansinfotab set LoansTitle=@LoansTitle ,Updatetime=@Updatetime,Url=@url,AuditID=@AuditID,RecommendID=@RecommendID where loansInfoID=@loansInfoID ";
            SqlParameter[] ps = new SqlParameter[]{
                     new SqlParameter("@LoansTitle",loansinfotab.LoansTitle)
                    ,new SqlParameter("@Updatetime",loansinfotab.Updatetime )
                    ,new SqlParameter("@url",loansinfotab.Url)
