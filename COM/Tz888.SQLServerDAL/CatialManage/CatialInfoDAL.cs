...
using System.Text;
using Tz888.Model.CatialManage;
using Tz888.DBUtility;
using System.Data;

namespace Tz888.SQLServerDAL.CatialManage
{
    public class CatialInfoDAL
    {
        //获取行业名称
        ...
        {
            string sql = "select count(InfoID) FROM CatialInfoTab where IndustryBID like '%" + IndustryBID + "%' ";
        ...
        {
            string sql = "select count(InfoID) from CatialInfoTab where ProvinceID like '%" + ProvinceID + "%' and  IndustryBID like '%" + IndustryBID + "%' ";
            return Convert.ToInt32(SearchDBHelper.GetSingle(sql));
        }
        ...
        {
            string sql = "select count(InfoID) FROM CatialInfoTab where ProvinceID like '%" + ProvinceID + "%' ";
            return Convert.ToInt32(SearchDBHelper.GetSingle(sql));
        }
        ...
        public DataTable GetCatialInfoIndustryBID(int PageCurrent,string IndustryBID)
        {
            ...
            + "[CapitalName],[ValidateTerm],[ComAbout] ,[AuditingStatus],[CountryCode] from CatialInfoTab where "
            + "IndustryBID like '%" + IndustryBID + "%' and infoID not in (select top "
            + 10 * (PageCurrent - 1) + " infoID from CatialInfoTab where IndustryBID like '%"
        ...
        {
            string sql = "select top 10 [InfoID] ,[Title],[publishT],[Hit],[FixPriceID],[HtmlFile],[ProvinceID],[CountyID],[IndustryBID],[CityID],"
             + "[CapitalName],[ValidateTerm],[ComAbout] ,[AuditingStatus],[CountryCode] from CatialInfoTab"
             + " where ProvinceID like '%" + ProvinceID + "%' and infoID not in (select top " + 10 * (PageCurrent - 1) + " infoID from CatialInfoTab"
             + " where ProvinceID like '%" + ProvinceID + "%' order by publishT desc) order by publishT desc";
        ...
        public DataTable GetCatialInfoByIndustryBIDAndProvinceID(int PageCurrent,string IndustryBID,string ProvinceID)
        {
            string sql = "select top 10 [InfoID] ,[Title],[publishT],[Hit],[FixPriceID],[HtmlFile],[ProvinceID],[CountyID],[IndustryBID],[CityID],"
             + "[CapitalName],[ValidateTerm],[ComAbout] ,[AuditingStatus],[CountryCode] from CatialInfoTab"
             + " where ProvinceID like '%" + ProvinceID + "%' and  IndustryBID like '%" + IndustryBID + "%' and infoID not in (select top " + 10 * (PageCurrent - 1) + " infoID from CatialInfoTab"
             + " where ProvinceID like '%" + ProvinceID + "%' and  IndustryBID like '%" + IndustryBID + "%' order by  publishT desc   )  order by  publishT desc";
