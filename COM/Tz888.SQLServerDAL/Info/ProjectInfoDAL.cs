...
namespace Tz888.SQLServerDAL.Info
{
    ...
    public class ProjectInfoDAL : IProjectInfo
    {
        ...
        private const string SP_InfoResource_InsertNew = "Project_gq_InsertNewFZ";
        ...
        //新属性
        #region 债券 发布 第一步    #region 债券 发布 第一步
        ...
        public long delete(int infoId)
        {
            ...
            sql = "delete from  ProjectInfoTab where InfoID=@infoId";
            SqlParameter[] para1 ={ 
            ...
            if (DelCases > 0)
            {
                ...
                paraC[0].Value = infoId;
                InfoContact = (long)DbHelperSQL.ExecuteSql(sql, paraC);
                if (InfoContact > 0 || DelCases > 0)
                {
                    sql = "delete from InfoResourceTab where InfoID=@infoId";
                    SqlParameter[] paraI ={ 
                    new SqlParameter("@infoId",SqlDbType.VarChar,20)
                };
                    paraI[0].Value = infoId;
                    Info = (long)DbHelperSQL.ExecuteSql(sql, paraI);
                }
                if (InfoContact > 0 || DelCases > 0)
                {
                    ...
                    paraA[0].Value = infoId;
                    Audit = (long)DbHelperSQL.ExecuteSql(sql, paraA);
                    if (Audit > 0 || DelCases > 0)
                    {
        ...
        #region 债券第二次修改
        public bool ProjectInfoZQ_Update_Up(Tz888.Model.Info.ProjectSetModel model, List<Tz888.Model.Info.InfoResourceModel> infoResourceModels)
        {
            SqlParameter[] parameters = {
                                    //主表信息
					                new SqlParameter("@InfoID", SqlDbType.BigInt,8),
                               ...
                               //201006资源超市第二次改版，新增字段
                                ...
                                new SqlParameter("@FixPriceID",SqlDbType.VarChar,100),
            ...
            parameters[74].Value = model.MainInfoModel.Hit;
            parameters[75].Value = model.MainInfoModel.FixPriceID;
            parameters[76].Value = model.MainInfoModel.AuditingStatus;
            bool ReturnValue = false;
            using (SqlConnection sqlConn = DbHelperSQL.GetSqlConnection())
            {
                sqlConn.Open();
                ...
                try
                {
                    int rowsAffected;
                    //修改项目信息
                    DbHelperSQL.RunProcedure(sqlConn, sqlTran, "Project_Update_old", parameters, out rowsAffected);
        ...
        #endregion
        #region 股权第二次修改
        public bool ProjectInfoGQ_Update_Up(Tz888.Model.Info.ProjectSetModel model, List<Tz888.Model.Info.InfoResourceModel> infoResourceModels)
        {
            SqlParameter[] parameters = {
                                    //主表信息
					                new SqlParameter("@InfoID", SqlDbType.BigInt,8),
            ...
            parameters[71].Value = model.ProjectInfoModel.InformationIntegrity;    //--信息完整度
            //-------END---------------------------
            parameters[72].Value = model.MainInfoModel.Hit;
            parameters[73].Value = model.MainInfoModel.FixPriceID;
            parameters[74].Value = model.MainInfoModel.AuditingStatus;
            bool ReturnValue = false;
            using (SqlConnection sqlConn = DbHelperSQL.GetSqlConnection())
            {
                sqlConn.Open();
        ...
        public long InsertNew(MainInfoModel mainInfoModel, ProjectInfoModel projectInfoModel, InfoContactModel infoContactModel, List<Tz888.Model.Info.InfoResourceModel> infoResourceModels)
        {
            SqlParameter[] parameters = {
                ...
                //-----------------------资源联系信息--------------------------
                    ...
                    new SqlParameter("@WebSite", SqlDbType.VarChar,200),
                    new SqlParameter("@ManageTeamAbout", SqlDbType.VarChar,500),
                    new SqlParameter("@MainPointCount", SqlDbType.Float,8),
                    new SqlParameter("@AuditingStatus",SqlDbType.Int,4),
            ...
            parameters[53].Value = mainInfoModel.MainPointCount;
            parameters[54].Value = mainInfoModel.AuditingStatus;
            int rowsAffected;
            long infoID;
            using (SqlConnection sqlConn = DbHelperSQL.GetSqlConnection())
            {
        ...
        public string selInfoIdRegion(int beginfo, int endinfo)
        {
            ...
            string sql = "select a.InfoID from MainInfoTab as a inner join ProjectInfoTab as b on a.InfoID=b.InfoID where a.InfoID <=@endinfo and a.InfoID>=@beginfo  and a.AuditingStatus =1 order by a.publishT desc";
            SqlParameter[] para ={
