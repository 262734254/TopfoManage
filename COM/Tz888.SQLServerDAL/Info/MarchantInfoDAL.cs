...
namespace Tz888.SQLServerDAL.Info
{
    ...
    public class MarchantInfoDAL : IMarchantInfo
    {
        ...
        private const string SP_MarchantFZInfo_Insert = "MerchantInfoTabFZN_Insert";
            ...
            )
        {
            SqlParameter[] parameters = {
                //---------------------资源信息主体----------------------
					...
					new SqlParameter("@ValidateTerm", SqlDbType.Int,4),
					new SqlParameter("@TemplateID", SqlDbType.Char,10),
                    new SqlParameter("@HtmlFile", SqlDbType.NVarChar,100),
                ...
                //这里是2010-06-01 添加政府招商的参数 有5个参数
                 ...
                 //新添加结束处
                  ...
                  new SqlParameter("@AuditingStatus",SqlDbType.Int,8),
            ...
            parameters[65].Value = mainInfoModel.AuditingStatus;
            parameters[66].Value = mainInfoModel.MainPointCount;
            int rowsAffected;
            long infoID;
            using (SqlConnection sqlConn = DbHelperSQL.GetSqlConnection())
            {
                ...
                {
                    //插入招商资源信息插如方法
                    DbHelperSQL.RunProcedure(sqlConn, sqlTran, SP_MarchantFZInfo_Insert, parameters, out rowsAffected);
                    infoID = (long)parameters[0].Value;
                    if (infoID < 0)
                        throw new Exception();
        ...
        #region IMarchantInfo 成员


        public bool UpdateMerchantSet(MerchantSetModel model)
        {
            SqlParameter[] parameters = {
                                    //主表信息
					                new SqlParameter("@InfoID", SqlDbType.BigInt,8),
            ...
            //2010-06-11以下是招商联系人职位
            parameters[51].Value = model.InfoContactModel.Position;
            parameters[52].Value = model.MainInfoModel.Hit;
            parameters[53].Value = model.MainInfoModel.AuditingStatus;
            parameters[54].Value = model.MerchantInfoModel.Merchanreturns;
            parameters[55].Value = model.MainInfoModel.MainPointCount;
            parameters[56].Value = model.MainInfoModel.FixPriceID;
            //End
            ...
            using (SqlConnection sqlConn = DbHelperSQL.GetSqlConnection())
            {
                ...
                try
                {
                    int rowsAffected;
                    //修改招商信息
                    DbHelperSQL.RunProcedure(sqlConn, sqlTran, "MerchantInfoTab_UpdateNew", parameters, out rowsAffected);
