...
namespace Tz888.SQLServerDAL.Info.V124
{
    ...
    {
        private const string SP_CapitalInfoInfo_Insert = "CapitalInfoTabFZ_Insert";
        ...
        //以下是启用上传文件实体
        {
            ...
            parameters[65].Value = mainInfoModel.MainPointCount;
        ...
        public bool Update(Tz888.Model.Info.V124.CapitalSetModel model)
        {
            ...
            parameters[53].Value = model.MainInfoModel.Hit;
            parameters[54].Value = model.MainInfoModel.HtmlFile;
            //parameters[55].Value = model.MainInfoModel.FixPriceID;
            ...
            using (SqlConnection sqlConn = DbHelperSQL.GetSqlConnection())
            {
                ...
                try
                {
                    int rowsAffected;
                    //插入投资资源信息
                    DbHelperSQL.RunProcedure(sqlConn, sqlTran, "CapitalInfoTab_UpdateNew", parameters, out rowsAffected);
